...
def batch(problem, rng):
    ...
    indices, mask = poisson.subsample(rng, n, LOT_SIZE / n, b_max)
...
def test_noiseless_estimate_equals_the_core_chain(grad_fn, zero_params, batch,
    ...
    x_b, y_b, mask = batch
    got = dp_step.private_gradient(
        grad_fn, zero_params, x_b, y_b, mask, key,
        lot_size=LOT_SIZE, clip_norm=CLIP, noise_multiplier=0.0,
    )
    want = aggregation.average_over_batch(
        _clipped(grad_fn, zero_params, batch), LOT_SIZE, mask=mask
    )
...
def test_noise_is_added_to_the_sum_at_sigma_times_clip(grad_fn, zero_params,
    ...
    got = dp_step.private_gradient(
        grad_fn, zero_params, x_b, y_b, mask, key,
        lot_size=LOT_SIZE, clip_norm=CLIP, noise_multiplier=sigma,
    )
    mean = aggregation.average_over_batch(
        _clipped(grad_fn, zero_params, batch), LOT_SIZE, mask=mask
    )
    want = noise.add_gaussian(mean, key, sigma * CLIP / LOT_SIZE)
...
def test_sensitivity_is_bounded_by_clip_over_lot_size(grad_fn, zero_params,
    ...
    bound = CLIP * float(mask.sum()) / LOT_SIZE
...
def test_padding_does_not_contribute(grad_fn, zero_params, problem, key):
    ...
    padded = dp_step.private_gradient(
        grad_fn, zero_params, x[indices], y[indices], mask, key,
        lot_size=LOT_SIZE, clip_norm=CLIP, noise_multiplier=0.0,
    )
    exact = dp_step.private_gradient(
...
def test_the_lot_size_divisor_is_not_the_batch_length(grad_fn, zero_params,
                                                      batch, key):
    """L is a constant; a data-dependent divisor would leak."""
    x_b, y_b, mask = batch
    args = dict(lot_size=LOT_SIZE, clip_norm=CLIP, noise_multiplier=0.0)
    got = dp_step.private_gradient(grad_fn, zero_params, x_b, y_b, mask, key,
                                   **args)
    doubled = dp_step.private_gradient(
        grad_fn, zero_params, x_b, y_b, mask, key,
        **{**args, "lot_size": 2 * LOT_SIZE}
    )
...
def test_the_key_controls_the_noise(grad_fn, zero_params, batch):
    ...
    a = dp_step.private_gradient(grad_fn, zero_params, x_b, y_b, mask,
                                 jax.random.key(0), **args)
    same = dp_step.private_gradient(grad_fn, zero_params, x_b, y_b, mask,
                                    jax.random.key(0), **args)
    other = dp_step.private_gradient(grad_fn, zero_params, x_b, y_b, mask,
